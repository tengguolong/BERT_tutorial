...
class Trainer:
    ...
    def init_optimizer(self, lr):
        # 用指定的学习率初始化优化器
        self.optimizer = torch.optim.Adam(self.optim_parameters, lr=lr, weight_decay=1e-4)
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, start_epoch + train_epoches):
        ...
        trainer.save_state_dict(trainer.bert_model, epoch,
                                ...
                                file_path="news_cls.model")

        auc = trainer.test(epoch)

        all_auc.append(auc)
        best_auc = max(all_auc)
        print("best: {:.3f}\tlatest: {:.3f}".format(best_auc, all_auc[-1]))
