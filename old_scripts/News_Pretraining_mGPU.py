...
class Pretrainer:
    ...
    def iteration(self, epoch, data_loader, train=True, df_path=None):
        ...
        for i, data in data_iter:
            ...
            positional_enc = self.positional_enc[:, :data["bert_input"].size()[-1], :].cuda().repeat(num_devices, 1, 1)

            # 1. forward the next_sentence_prediction and masked_lm model
            mlm_preds = self.bert_model(input_ids=data["bert_input"], positional_enc=positional_enc, token_type_ids=None)
