...
config = dict(
    vocab_path = 'corpus/news_word2idx.json',
    vocab_size = 6874,
    num_workers = 0,
    train_corpus_path = 'corpus/news/trainval.csv',
    val_corpus_path = 'corpus/news/test_a.csv',
    test_corpus_path = 'corpus/news/test_a.csv',
)


config["pretrained"] = 'bert_state_dict/uncased_L-6_H-256_A-4/pytorch_model.bin'
config["output_path"] = "save/pretrain_L6_H256_A4"
...
class Pretrainer:
    def __init__(self,
        ...
        self.bert_model = BertMLMPretrain(bertconfig)
        ...
        train_dataset = BERTDataset(corpus_path=config["train_corpus_path"],
                                    word2idx_path=config["vocab_path"],
        ...
        warmup_iters = num_iter_per_epoch * 2
